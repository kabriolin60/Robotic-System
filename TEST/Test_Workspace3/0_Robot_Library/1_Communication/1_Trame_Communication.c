 ...
 **
 *****************************************************************************/
static TO_AHBS_RAM3 uint8_t Data_rx[COMMUNICATION_TRAME_MAX_DATA + 11];
...
__attribute__((optimize("O0"))) BaseType_t _1_Communication_Create_Trame_From_Buffer(RINGBUFF_T *RingBuff)
{
	...
	crc += (uint8_t)received_trame.Instruction;

	received_trame.Slave_Adresse = Data_rx[5];
	crc += (uint8_t)received_trame.Slave_Adresse;

	received_trame.Length = (uint8_t)(API_LENGTH - 7);
	...
	if (received_trame.Length <= COMMUNICATION_TRAME_MAX_DATA)
	{
		...
		{
			crc += (uint8_t)(received_trame.Data[index]);
		}
