...
unsigned write_flash(unsigned int dst, char * src, unsigned no_of_bytes)
{
	unsigned i;
...
void Write_Bloc(unsigned int flash_address)
{
	if(flash_address < sector_start_map[USER_START_SECTOR])
	{
		while(1);
	}

	// We have accumulated enough bytes to trigger a flash write
	find_erase_prepare_sector(SystemCoreClock/1000, flash_address);
	...
	write_data(SystemCoreClock/1000, flash_address, (unsigned *)flash_buf ,FLASH_BUF_SIZE);
