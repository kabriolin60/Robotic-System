...
void vTaskList( char * pcWriteBuffer )
{
	...
	*pcWriteBuffer = 0x00;
	...
	{
		sprintf(( char * ) pcWriteBuffer, "\r\rFreeBytesRemaining = %ld\r****************************************************************\r", xFreeBytesRemaining);
		pcWriteBuffer += strlen( ( char * ) pcWriteBuffer );

		sprintf(( char * ) pcWriteBuffer, "Task                  Status      Priority        Stack       Num\r****************************************************************\r");
		pcWriteBuffer += strlen( ( char * ) pcWriteBuffer );
		...
		for( x = 0; x < uxArraySize; x++ )
		{
			...
			sprintf( ( char * ) pcWriteBuffer, ( char * ) "%-12s\t%c\t%u\t%u\t%u\r", pxTaskStatusArray[ x ].pcTaskName, cStatus, ( unsigned int ) pxTaskStatusArray[ x ].uxCurrentPriority, ( unsigned int ) pxTaskStatusArray[ x ].usStackHighWaterMark, ( unsigned int ) pxTaskStatusArray[ x ].xTaskNumber );
			pcWriteBuffer += strlen( ( char * ) pcWriteBuffer );
		}
...
void vTaskGetRunTimeStats( char *pcWriteBuffer )
{
	...
	*pcWriteBuffer = 0x00;
	...
	if( pxTaskStatusArray != NULL )
	{
		...
		if( ulTotalTime > 0 )
		{
			sprintf(( char * ) pcWriteBuffer, "\rTask              Abs Time         %% \r*****************************************\r");
			pcWriteBuffer += strlen( ( char * ) pcWriteBuffer );
			...
			for( x = 0; x < uxArraySize; x++ )
			{
				...
				ulStatsAsPercentage = (float)((float)pxTaskStatusArray[ x ].ulRunTimeCounter / (float)ulTotalTime);

				if( ulStatsAsPercentage > 0 )
				{
#ifdef portLU_PRINTF_SPECIFIER_REQUIRED
					{
						sprintf( ( char * ) pcWriteBuffer, ( char * ) "%-12s\t%-7u\t\t%.1f%%\r", pxTaskStatusArray[ x ].pcTaskName, pxTaskStatusArray[ x ].ulRunTimeCounter, ( float ) ulStatsAsPercentage );
					}
#else
					{
						/* sizeof( int ) == sizeof( long ) so a smaller
											printf() library can be used. */
						sprintf( ( char * ) pcWriteBuffer, ( char * ) "%-12s\t%-7u\t\t%.1f%%\r", pxTaskStatusArray[ x ].pcTaskName, ( unsigned int ) pxTaskStatusArray[ x ].ulRunTimeCounter, ( float ) ulStatsAsPercentage );
					}
#endif
				}
				else
...
#ifdef portLU_PRINTF_SPECIFIER_REQUIRED
					{
						sprintf( ( char * ) pcWriteBuffer, ( char * ) "%-12s\t%-7u\t\t0%%\r", pxTaskStatusArray[ x ].pcTaskName, pxTaskStatusArray[ x ].ulRunTimeCounter );
					}
#else
					{
						/* sizeof( int ) == sizeof( long ) so a smaller
											printf() library can be used. */
						sprintf( ( char * ) pcWriteBuffer, ( char * ) "%-12s\t%-7u\t\t0%%\r", pxTaskStatusArray[ x ].pcTaskName, ( unsigned int ) pxTaskStatusArray[ x ].ulRunTimeCounter );
					}
#endif
				}
